...
public class PlayerController : MonoBehaviour
{
    ...
    {
        // Player Inventory
        if (Input.GetKeyDown(KeyCode.Q) && _isSpeaking == false && Vector3.Distance(transform.position, movePoint.position) <= 0.05f) // Can only open inventory when: "Q" Key is pressed, no active dialogue box, is not moving
        {
            MenuManager.instance.ToggleMenu(!_menuState);
        ...
        if (_movementState)
        {
            ...
            if (Vector3.Distance(transform.position, movePoint.position) <= 0.05f) // Only accept inputs if the player is not moving / not far from the Move Point (the difference in position gives leeway to the player's movement inputs)
            {
